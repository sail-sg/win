...
class Lamb_Win(Optimizer):
    ...
    def __init__(
            self, params, lr=1e-3, bias_correction=True, betas=(0.9, 0.999), reckless_steps=(2.0, 8.0), eps=1e-6,
            weight_decay=0.01, grad_averaging=True, max_grad_norm=1.0, trust_clip=False, 
            always_adapt=False, acceleration_mode=None):
        defaults = dict(
            lr=lr, bias_correction=bias_correction, betas=betas, reckless_steps=reckless_steps, eps=eps, weight_decay=weight_decay,
            grad_averaging=grad_averaging, max_grad_norm=max_grad_norm, trust_clip=trust_clip, 
            always_adapt=always_adapt, acceleration_mode=acceleration_mode)
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if 'win' in group['acceleration_mode']:
                    beta4, beta5 = group['reckless_steps']
